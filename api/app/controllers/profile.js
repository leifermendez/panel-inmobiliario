...
const importDummy = require('../service/importDummy')
 ...
 * @param {string} id - user id
 */
const updateStepperInDB = async (req, id) => {
  return new Promise((resolve, reject) => {
    model.findByIdAndUpdate(
      id,
      {
...
/**
 * Update stepper function called by route
 * @param {Object} req - request object
 * @param {Object} res - response object
 */
exports.updateStepper = async (req, res) => {
  try {
    const id = await utils.isIDGood(req.user._id)
    req = matchedData(req)
    res.status(200).json(await updateStepperInDB(req.stepper, id))
  } catch (error) {
    utils.handleError(res, error)
  }
}
/**
 * Import dummy data stepper function called by route
 * @param {Object} req - request object
 * @param {Object} res - response object
 */
exports.importDummy = async (req, res) => {
  try {
    const dataUser = req.user;
    const acc = await importDummy.generateAccommodation(dataUser._id)
    await importDummy.generateTask(dataUser._id, acc._id)
    await importDummy.generateBooking(dataUser._id, acc._id)
    await importDummy.generateNotification(dataUser._id)
